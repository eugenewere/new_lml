...
def mark_as_read_or_unread(request):
    if request.method == 'POST':
        ...
        for i in list1:
            message = Message.objects.filter(id=int(i)).first()
            if Message.objects.filter(reciever=user, message=message,readstatus='UNREAD'):
                Message.objects.filter(reciever=user, message=message).update(
                    readstatus='READ'
                )
            elif Message.objects.filter(reciever=user, message=message,readstatus='READ'):
                Message.objects.filter(reciever=user, message=message).update(
                    readstatus='UNREAD'
                )
